...
def test_postgres_integration():
    ...
    try:
        ...
        assert wait_for("http://localhost:8000/health" ) or wait_for("http://localhost:8000/docs"), "app did not start"

        # Post a log
        resp = requests.post("http://localhost:8000/logs", json={"message": "integration test critical panic"})
