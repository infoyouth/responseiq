from src.services.analyzer import analyze_message, analyze_log


def test_simple_keyword_severity_mapping():
    # panic/critical -> high
    for m in ("panic: something broke", "critical failure occurred"):
        meta = analyze_message(m)
        assert meta is not None
        assert meta.get("severity") == "high"

    # error/failed -> medium
    for m in ("An error occurred", "operation failed due to X"):
        meta = analyze_message(m)
        assert meta is not None
...
def test_event_keyword_mapping():
    # event-level analyzer (analyze_log) should map oomkilled and crashloop to high
