...
2. ``find_similar(incident_id, threshold)`` loads all stored embeddings,
   computes cosine similarity in pure Python (no numpy required), and
...
To switch to pgvector:
  - Replace the ``embedding_json`` TEXT column with ``VECTOR(1536)``.
  - Replace the Python cosine loop with an ``ORDER BY embedding <=> query``
    SQL expression.
